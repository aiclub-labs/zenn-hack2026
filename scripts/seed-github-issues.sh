...
## 背景
Zennハッカソンへのエントリー提出と、法人社内承認・利益相反チェックを完了させる。提出が遅れるとエントリーセッション(5/14)案内が来ない。

## チェックリスト
- [ ] **A1** Zennエントリーフォーム提出（法人法人区分・チーム代表）
- [ ] **A2** Zenn org `ai-club` ハンドル予約（fallback: `aiclub-jp` / `aiclub-jp`）
- [ ] **A3-2** 法人帰属表記の許可確認（マーケ）
