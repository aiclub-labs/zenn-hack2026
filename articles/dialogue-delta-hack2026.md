...
Microsoft Agent Hackathon Japan 2026 (Zenn) 向けに、**Azure Container Apps + AOAI (gpt-5.4 + gpt-5.4-mini + text-embedding-3-small) + Cosmos DB + AI Search** で実装しました。

## まずは 3 分のデモ動画 (overview)
...
ユーザー → Static UI (Container Apps + nginx) ──→ FastAPI (Container Apps)
                                       ...
                                       text-embed-3)   12 container) keyword)
```
...
```text
gap = (self_critic < 3.0)
   OR (3.0 <= self_critic <= 9.5 AND distance > 0.15)
```
...
- 各 claim を ensemble (3 LLM 構成: large@0.0 / small@0.2 / large@0.6) で真偽判定 → 多数決で `supported` / `conflict` / `novel` に集約
- verdict が `conflict` なら conflict queue 行き、それ以外なら reviewer 承認とともに corpus.upsert (AI Search に embedding 投入)
...
corpus 文書自体に `upvotes` / `downvotes` / `score` (= upvotes − downvotes) フィールドを持たせ、Good / Bad ボタンで read-modify-write しています。`/popular` ページでは `score` 順に並べるだけで ranking が出るので、ソート用のカスタムスコアリングプロファイルは不要です。上位記事は **「全社で共有する価値あり = general に昇格させる候補」** として意味付けし、reviewer の手動 / 自動移管経路は後述の Phase 3 に設計余白として残しました。
...
提出のタイミングでもう 1 段、**Microsoft Easy Auth + Entra アプリ登録 (multi-tenant + MSA)** を Container App 入口に被せ、未認証 URL アクセスを login.microsoftonline.com に強制リダイレクトするようにしました。**API 側 Container App は ingress を internal-only にし、UI の nginx が同 Environment 内から HTTP proxy で呼ぶ経路のみ残した** ことで、API URL の直叩きを物理的に遮断しています。
...
7. **テナント切替 + 認証** — Combobox で sector/unit を切替えられ、Microsoft Easy Auth (multi-tenant + 個人 MSA) で入口を守っています
...
- 「**人の頭から汲み取る (hearout)**」+「**人が能動的に書く (post)**」+「**皆が引く (chat citation)**」+「**皆が評価する (vote / ranking)**」の 4 経路で knowledge graph が育っていきます
...
- **Claude Code (Opus 4.7)** をペアプログラマとして全工程で活用、特に UI/UX のモバイル対応と UAT 自動化 (Playwright MCP) が高速化
- **GitHub Actions × Claude Code 連携** — `@claude` メンションで PR / issue コメントから Claude Code が編集・応答 (`.github/workflows/claude.yml`)
- **Discord 通知の集約** — アプリ内 SLA アラート (`notification` agent) と CI / デプロイ結果通知 (`_notify-discord.yml`) を同一サーバーに集約
- **Bicep による Azure IaC** — Container Apps / AOAI / Cosmos / AI Search を Bicep で記述、`infra-deploy.yml` で PR ベースデプロイ
- ブランチ戦略: `feat/domain-pivot-consultant-tacit` に集約、main へは提出直前にマージ

self-critic 文化はチームにも持ち込みました。「これは自信ある / これは曖昧」を PR レビューで明示するよう統一し、self-critic で AI に持たせた規律と人間チームが互いに持つ規律を揃えています。

開発プロセスそのものにも **Human-in-the-loop を意図的に組み込みました**。Discord / Azure / GitHub / Claude Code を繋ぎ込んで、機械が引き受けられる部分 (実装下書き、UAT 自動化、デプロイ、通知、CI からのレビュー応答) を最大限自動化し、**人間は要件判断と PR レビューに時間を集中** する形にしています。ハッカソンの限られた時間を、技術的な詰めではなく方針判断と品質ゲートに振れたことが、Phase 1 MVP の完成度に直結しました。
