...
**判定基準**: 観客が「3 軸スコアリング + 双方向」が独自と即理解できるか / 法人 文脈なしでも汎用 talent matching agent として動くか。
...
**他チームと被るリスク**: talent matching は被る確率中 → 上記 3 点 + 法人 固有（KC Bridge + GPDR）で diff を出す。
