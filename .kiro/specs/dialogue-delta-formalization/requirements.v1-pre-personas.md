...
対象組織 AI 部の組織的暗黙知活用の POC として位置付ける。
...
### 用語: 暗黙知の定義（2026-05-20 meeting 決定事項）
...
### Persona と Requirement のトレーサビリティ

本 requirements は [`personas-stories.md`](./personas-stories.md) で定義した 3 persona の User Story を起点に組み立てている。

| Persona | 役割（2026-05-20 meeting 決定の整理） | 主要 Story | 対応 Requirement |
|---|---|---|---|
| **A: 知識管理者** (シゲル) | 暗黙知を形式化したい人 | A1, A2, A3 | Req 1（スキーマ定義）, Req 9（多マス対応） |
| **B: 業務ユーザー** (ハルカ) | 暗黙知を借りる/活用する人（5W1H ヒアリング協力者でもある） | B1, B2, B3, B4, B5 | Req 2（対話監視）, Req 3（差分検知）, Req 4（5W1H ヒアリング）, Req 8（corpus 参照）, Req 11.3（redact） |
| **C: レビュアー** (タケシ) | C1-C3 はレビュー action のバリエーション（承認/編集/拒否、いずれか1つを選択）/ **C4 は矛盾検知を担う人間** | C1, C2, C3, C4, C5 | Req 5（重み付き形式化）, Req 6（HITL レビュー）, Req 7（矛盾検知 / Truth Judgment） |

横断要件（Persona 非依存）: Req 10（観測性）, Req 11（セキュリティ）, Req 12（予算管理）

### 業務フロー絞り込み方針（2026-05-20 meeting 決定）

現段階では**業務フローを絞らず汎用アプリケーション**として設計する。具体業務フロー（人材マッチング / 資料検索 等）への絞り込みは **design 段階で再判断**する（参照: [`risks.md`](../../../docs/risks.md) R-07）。

### リスク台帳との連動

各 Requirement が緩和対象とするリスクは [`../../../docs/risks.md`](../../../docs/risks.md) を参照:

- **R-01**（供給側インセンティブ）: Req 4（skip 自由度）, Req 8（引用可視化による評価フィードバック）
- **R-02**（確からしさ / 重み付け）: Req 5（重み付き形式化）, Req 7（Truth Judgment）
- **R-03**（矛盾検知 2 パターン）: Req 7（入力時 / C4）, Req 8.4（活用時 / B5）
- **R-04**（高精度モデル可否）: Req 12（モデル切替提案）
- **R-06**（需要側インセンティブ）: Req 4（skip 可 modal）, Req 8（過去 record 引用）
- **R-07**（業務フロー絞り込み）: design 段階で確定（本 requirements は汎用前提）

## Requirements

### Requirement 1: 暗黙知スキーマの事前定義

**Objective:** As an admin (対象組織 AI 部の知識管理担当), I want セクター×ユニット単位で「捕捉したい暗黙知データ項目」を事前定義する, so that 対話監視中のエージェントが暴走せずスキーマに沿った差分のみ検出できる
